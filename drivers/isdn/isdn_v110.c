...
/* Die folgenden Daten sind fertig kodierte Matrizen, jeweils
   als online und offline matrix f?r 9600, 19200 und 38400
 */
...
/* ValidHeaderBytes pr?ft, wieviele bytes in v->decodebuf g?ltig sind */
...
/* SyncHeader schiebt den decodebuf pointer auf den n?chsten g?ltigen header */
...
/*
  ...
  return buflen;                                                                                                     anzahl schon erzeugter daten zur?ckgeben!
  */
			}
			line++; /* sonst die n?chste matrixzeile nehmen */
			continue;
		} else if ((line % 10) == 5) {	/* in zeile 5 stehen nur e-bits ! */
			if ((m[line] & 0x70) != 0x30) {	/* 011 mu? am anfang stehen! */
#ifdef ISDN_V110_DEBUG
 ...
 */
			...
			if (mbit > 2) {	/* war es das letzte bit dieser matrixzeile ? */
				mbit >>= 1;	/* nein, nimm das n?chste in dieser zeile */
				continue;
			}       /* sonst links in der n?chsten zeile anfangen */
			mbit = 64;
			line++;
			continue;
		} else {        /* sonst m?ssen wir ein datenbit setzen */
			if (m[line] & mbit)	/* war das bit in der matrix gesetzt ? */
				b |= dbit;	/* ja, dann setz es auch im datenbyte  */
			else
				b &= dbit - 1;	/* nein, l?sch bit im datenbyte */
			if (dbit < 128)	/* haben wir schon ein ganzes byte voll ? */
				dbit <<= 1;	/* nein, auf zum n?chsten datenbit */
				...
				dbit = 1;	/* als n?chstes suchen wir das nullte bit */
			}
			goto next_byte;	/* suche das n?chste bit in der matrix */
		}
	}
...
/* DecodeStream erh?lt vom input stream V110 kodierte Daten, die zu den
   V110 frames zusammengepackt werden m?ssen. Die Daten k?nnen an diese
   Schnittstelle so ?bergeben werden, wie sie von der Leitung kommen, ohne
   darauf achten zu m?ssen, das frames usw. eingehalten werden.
 */
struct sk_buff *
isdn_v110_decode(isdn_v110_stream * v, struct sk_buff *skb)
{
	...
	if (ValidHeaderBytes(v) != v->nbytes) {	/* ist es ein ung?ltiger header ? */
...
EncodeMatrix(unsigned char *buf, int len, unsigned char *m, int mlen)
{
	...
	next_bit:
		switch (mbit) { /* ganz linkes oder rechtes bit ? */
			case 1:
				line++;	/* ganz rechts ! dann in die n?chste */
				if (line >= mlen) {
					printk(KERN_WARNING "isdn_v110 (EncodeMatrix): buffer full!\n");
					return line;
				}
		...
		if (dbit == 128) {	/* war es das letzte datenbit ? */
			dbit = 1;	/* dann mach beim n?chsten weiter */
			i++;    /* n?chste datenbyte des input buffers */
			...
			dbit <<= 1;	/* dann gehe zum n?chsten datenbit */
